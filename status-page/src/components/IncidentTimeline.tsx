...
export function IncidentTimeline({ incidents, showResolved = false }: IncidentTimelineProps) {
    ...
    <div className="space-y-6">
      {incidents.map((incident) => (
        ...
        >
          ...
          <ol className="mt-4 space-y-4 border-t border-zinc-100 pt-4 dark:border-zinc-800">
            {incident.updates.map((update, index) => (
              <li key={update.id} className="relative pl-6">
                {index < incident.updates.length - 1 && (
                  <span
                    className="absolute left-[7px] top-5 h-full w-px bg-zinc-200 dark:bg-zinc-700"
                    aria-hidden
                  />
                )}
                <span
                  className="absolute left-0 top-1.5 h-3.5 w-3.5 rounded-full border-2 border-zinc-300 bg-white dark:border-zinc-600 dark:bg-zinc-900"
                  aria-hidden
                />
